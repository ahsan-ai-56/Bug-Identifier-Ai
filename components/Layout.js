...
export function Nav({ active }) {
  ...
  const [mobToolsOpen, setMobToolsOpen] = useState(false);

  return (
    <>
      <nav className="nav">
        <Link href="/" className="nav-logo">
          <img src="/favicon2.png" alt="Bug Identifier" width="32" height="32" />
        ...
        <ul className="nav-links">
          ...
          {/* Identifier dropdown — hover pe khulta hai (desktop) */}
          <li className="nav-drop">
            <span className={`nav-drop-label${active === "tools" ? " on" : ""}`}>
              Identifiers <span className="caret">▾</span>
            </span>
            <div className="nav-drop-menu">
              <div className="nav-drop-grid">
                {ALL_TOOLS.map(t => (
                  <Link key={t.href} href={t.href} className="nav-drop-item">
                    <span className="ico">{t.emoji}</span> {t.label}
                  </Link>
                ))}
              </div>
            </div>
      ...
      <div className={`mob-menu${open ? " open" : ""}`}>
        ...
        <div className="mob-tools-toggle" onClick={() => setMobToolsOpen(!mobToolsOpen)}>
          🔧 All Identifiers <span>{mobToolsOpen ? "▲" : "▼"}</span>
        </div>
        {mobToolsOpen && (
          <div className="mob-tools-list">
            {ALL_TOOLS.map(t => (
              <Link key={t.href} href={t.href} onClick={() => setOpen(false)}>{t.emoji} {t.label}</Link>
            ))}
          </div>
        )}
...
export function RelatedTools({ current }) {
  const all = [{ href: "/", label: "Bug Identifier", emoji: "🐛" }, ...ALL_TOOLS]
